...
from optimhc.parser.pin import _read_single_pin_as_df, read_pin
...
class TestPinParser:
    ...
    def test_read_pin_fragpipe_to_container(self, fragpipe_pin_path):
        ...
        assert container.psms.iloc[0][container.label_column]
        assert container.psms.iloc[1][container.label_column]
        assert container.psms.iloc[2][container.label_column]
        assert not container.psms.iloc[3][container.label_column]
    ...
    def test_parse_specid(self, fragpipe_pin_path, percolator_pin_path):
        ...
        assert (
            "LP20210421_HT_DCfeeding_Exp1_HLAII_0hr_fxn01.5401.5401.4_1"
            in container_fp.psms["SpecId"].values
        )

        container_perc = read_pin(percolator_pin_path)
        assert (
            "/media/kevin/Elements/MhcValidator_analysis/data/JY_serial_dilution/IP0040_11MAI2022_JY_MHC1_HUMAN_S4_2PELLETS_KK_SERIAL_DIL_PT_3_R2_3_2_1"
            in container_perc.psms["SpecId"].values
        )
