...
def test_strip_flanking_and_charge():
    ...
    assert (
        utils.strip_flanking_and_charge("F.VTVQGRAIC[119.0041]SDPNNKRVKN4.A")
        == "VTVQGRAIC[119.0041]SDPNNKRVKN"
    )
...
def test_remove_modifications():
    ...
    assert (
        utils.remove_modifications(pep, keep_modification="15.9949")
        == "AANDAGYFNDEM[15.9949]APIEVKTK"
    )
    assert (
        utils.remove_modifications(pep, keep_modification=["42.0106"])
        == "AANDAGYFNDEMAPIEVK[42.0106]TK"
    )
