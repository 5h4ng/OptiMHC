...
See the [changelog](CHANGELOG.md) for current breaking changes and migration notes.
...
| `SpectralSimilarity` | `mzmlDir: ./data`<br>`model: AlphaPeptDeep_ms2_generic`<br>`collisionEnergy: 28`<br>`instrument: LUMOS`<br>`tolerance: 20`<br>`numTopPeaks: 36`<br>`url: koina.wilhelmlab.org:443`<br>`ssl: true` | Computes similarity between experimental and predicted spectra. Each PSM `run` resolves to `<mzmlDir>/<run>.mzML`, and `scan` selects the spectrum. The `tolerance` parameter (10-50 ppm) controls peak matching. Optional: `url` — Koina server address (host:port); `ssl` — default true. |
...
| `seed`     | Integer | `1`          | Random seed passed to both the selected model and Mokapot training.                                                                                                                                        |
